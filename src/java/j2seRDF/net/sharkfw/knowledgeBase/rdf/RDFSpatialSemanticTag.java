...
public class RDFSpatialSemanticTag extends RDFSemanticTag implements SpatialSemanticTag {
		...
		for (int i = 0; i < si.length; i++) {
			...
			try {
				...
				m.getResource(si[i]).addProperty(
						m.createProperty(RDFConstants.SPATIAL_TAG_PREDICATE),
						m.createResource(si[i]
								+ RDFConstants.SPATIAL_TAG_OBJECT_NAME_GEOMETRY));

					m.createResource(
							si[i] + RDFConstants.SPATIAL_TAG_OBJECT_NAME_GEOMETRY)
							.addProperty(
									m.createProperty(RDFConstants.SPATIAL_TAG_EWKT),
									sg.getEWKT());
					
					m.createResource(
							si[i] + RDFConstants.SPATIAL_TAG_OBJECT_NAME_GEOMETRY)
							.addProperty(
									m.createProperty(RDFConstants.SPATIAL_TAG_WKT),
									sg.getWKT());
					
					m.createResource(
							si[i] + RDFConstants.SPATIAL_TAG_OBJECT_NAME_GEOMETRY)
							.addProperty(
									m.createProperty(RDFConstants.SPATIAL_TAG_SRS),
	...
	public RDFSpatialSemanticTag(RDFSharkKB kb, String si) throws SharkKBException {
		...
		Model m = dataset.getNamedModel(RDFConstants.SPATIAL_MODEL_NAME);	
		StmtIterator resultWKT = m.listStatements(m.getResource(si+ RDFConstants.SPATIAL_TAG_OBJECT_NAME_GEOMETRY),
				m.getProperty(RDFConstants.SPATIAL_TAG_WKT),
				(String) null);		
		dataset.end();
