...
public class TimestampList {
    ...
    protected List<PeerTimestamp> timestamps;
    protected SyncKB syncKB;
    ...
    public TimestampList(PeerSTSet peersToSyncWith, SyncKB kb) {
        ...
        while (peerEnum.hasMoreElements()) {
            timestamps.add(new PeerTimestamp(peerEnum.nextElement()));
        }
    }
    ...
    public void newPeer(PeerSemanticTag peer) {
        timestamps.add(new PeerTimestamp(peer));
        persist();
    }
    ...
    public PeerSTSet getPeers() {
        ...
        for (PeerTimestamp s : timestamps) {
            try {
    ...
    public void setAllTimestampsNull() {
        for (PeerTimestamp p : timestamps) {
            p.setTimestampNull();
        }
        persist();
    }
    ...
    private PeerTimestamp findPeerTimestamp(PeerSemanticTag p) {
        ...
        Iterator<PeerTimestamp> i = timestamps.iterator();
        PeerTimestamp waldo;
        // Lets look for waldo
        while(i.hasNext()) {
            waldo = i.next();
    ...
    protected void persist() {
        try {
            syncKB.setProperty(LIST_PROPERTY, serializeTimestamps());
    ...
    protected void retrieve() {
        ...
        String x = null;
        try {
            x = syncKB.getProperty(LIST_PROPERTY);
        } catch (SharkKBException ex) {
            // TODO
        }
    ...
    private String serializeTimestamps() {
        ...
        for(PeerTimestamp t : timestamps)
    ...
    private void deserializeTimestamps(String x) {
        for(String s : x.split(PeerTimestamp.PEER_TIMESTAMP_CLOSING_TAG)){
            try {
                timestamps.add(new PeerTimestamp(s));
            } catch (SharkKBException ex) {
    ...
    class PeerTimestamp {
        ...
        public PeerTimestamp(String serialized) throws SharkKBException {
            ...
            _peer = TimestampList.this.syncKB.getPeerSTSet().getSemanticTag(sis);
